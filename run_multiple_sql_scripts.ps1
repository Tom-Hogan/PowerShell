...
$input_path  = 'C:\_SQL_Queries\';          # specify location that holds the SQL scripts to be run
$output_path = 'C:\_SQL_Queries\_Output\';  # specify location for log files
...
ForEach($item in $server_list) { 
    try {
        ...
        ForEach($file in $script_list) {
            ...
            Invoke-Sqlcmd -ServerInstance $item -Database 'master' -InputFile $input_file -ErrorAction Stop;
